class Zen_PaintVehiclesGreen extends RecipeBase
{
	...
	override void Do(ItemBase ingredients[], PlayerBase player, array<ItemBase> results, float specialty_weight)//gets called upon recipe's completion
	{
		...
		cfgToFind.Replace("_OrangeRust", "");
		cfgToFind.Replace("_Orange", "");
		

		// Green is base type for these vehicles so don't add any color to them

		ItemBase paintedItem = ItemBase.Cast(GetGame().CreateObjectEx(cfgToFind, ingredients[1].GetPosition(), ECE_PLACE_ON_SURFACE));
		if (paintedItem)
		{
