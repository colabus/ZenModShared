class ZenHeliZombiesTrigger extends Trigger
{
	bool m_ShouldSpawn;
	bool m_AggroZombies;
	...
	void SetupZenHeliZombiesTrigger(int crashLifetime, int triggerRadius, int zombieCount, array<string> zedTypes, int spawnRadius, bool aggro = true)
	{
		ZenModLogger.Log("SetupZenHeliZombiesTrigger @ " + GetPosition() + " life=" + crashLifetime + " radius=" + triggerRadius + " zedCount=" + zombieCount, "triggers");
	...
	void ~ZenHeliZombiesTrigger()
	{
		ZenModLogger.Log("SetupZenHeliZombiesTrigger @ " + GetPosition() + " - ~DESTROY()", "triggers");
	...
	void StopSpawn()
	{
		ZenModLogger.Log("SetupZenHeliZombiesTrigger @ " + GetPosition() + " - StopSpawn()", "triggers");
	...
	override void OnEnter(Object obj)
	{
		...
		ZenModLogger.Log("SetupZenHeliZombiesTrigger @ " + GetPosition() + " - OnEnter() - " + player.GetIdentity().GetName() + "/" + player.GetIdentity().GetPlainId(), "triggers");
		SpawnRandomZombies();
	};
	...
	void SpawnRandomZombies()
	{
		...
		for (int i = 0; i < m_ZombieCount; i++)
		{
			...
			if (!zed)
			{
				ZenModLogger.Log("SetupZenHeliZombiesTrigger @ " + GetPosition() + " - FAILED TO CREATE ZOMBIE - " + randomZed, "triggers");
				continue;
			}
		...
		ZMPrint("[SetupZenHeliZombiesTrigger] Spawned zeds @ " + spawnPos);
		StopSpawn();
	}
};