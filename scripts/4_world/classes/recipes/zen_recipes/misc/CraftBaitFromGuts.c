class CraftBaitFromGutsRecipe extends RecipeBase
{
	override void Init()
	{
		m_Name = "Get fishing bait";
		...
		InsertIngredient(0, "SmallGuts");//you can insert multiple ingredients this way
		...
		//result1
		AddResult("ZenGutBait");//add results here
	...
	{
		int maxWorms = Math.RandomIntInclusive(9, 14);
		for (int i = 0; i < maxWorms; i++)
			GetGame().CreateObjectEx("ZenGutBait", player.GetPosition(), ECE_PLACE_ON_SURFACE);
	}
}
