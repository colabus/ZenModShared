class Zen_PaintCrossbowBlackRecipe extends RecipeBase   // Our example recipe which uses primary ingredient - our custom item MagicHammer to turn any other secondary ingredient ( all ItemBase items ) into stones. 
{
	override void Init()
	{
		int cost = 25;
		string useItem = "Spraycan_Black";
		string resultItem = "Crossbow_Black";
		
		
		m_Name = "Paint Crossbow Black";	// action name in game
		...
		m_MinQuantityIngredient[0] = cost;	//quantity required for primary ingredient
		...
		m_IngredientAddQuantity[0] = -cost;// -1 = do nothing
		m_IngredientDestroy[0] = false;
		m_IngredientUseSoftSkills[0] = false;	// set 'true' to allow modification of the values by softskills on this ingredient
		
		//ingredient 2	
		InsertIngredient(1, "Crossbow"); //  secondary ingredient
		InsertIngredient(1, "Crossbow_Green");
		InsertIngredient(1, "Crossbow_Autumn");
		InsertIngredient(1, "Crossbow_Summer");
		InsertIngredient(1, "Crossbow_Wood");
		...
		//result
		AddResult("Crossbow_Black");	// recipe result
	...
	override void Do(ItemBase ingredients[], PlayerBase player, array<ItemBase> results, float specialty_weight)//gets called upon recipe's completion
	{
		...
		{
			ItemBase paintedItem = results.Get(0);
			if (paintedItem)
			{
