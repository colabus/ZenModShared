class Zen_PaintCamoRecipes extends RecipeBase
{
	...
	{
		if (ingredients[1].GetType().Contains("Black") && ingredients[0].GetType().Contains("Green"))
			return true;

		if (ingredients[1].GetType().Contains("Green") && ingredients[0].GetType().Contains("Black"))
			return true;
	...
	override void Do(ItemBase ingredients[], PlayerBase player, array<ItemBase> results, float specialty_weight)//gets called upon recipe's completion
	{
		...
		ItemBase paintedItem = ItemBase.Cast(GetGame().CreateObjectEx(cfgToFind + "_Camo", ingredients[1].GetPosition(), ECE_PLACE_ON_SURFACE));
		if (paintedItem)
		{
